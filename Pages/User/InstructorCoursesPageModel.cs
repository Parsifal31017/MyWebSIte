...
namespace MyWebSite.Pages.User
{
    public class InstructorCoursesPageModel : PageModel
    {
        ...
        public List<AssignedAdsData> AssignedAdsDataList;

        public void PopulateAssignedCourseData(ApplicationDbContext context,
                                               MyWebSite.Models.User instructor)
        {
            var allCourses = context.Ads;
            var instructorAds = new HashSet<int>(
                instructor.AdsAssignments.Select(c => c.AdsID));
            AssignedAdsDataList = new List<AssignedAdsData>();
            foreach (var ads in allCourses)
            {
                AssignedAdsDataList.Add(new AssignedAdsData
                {
                    AdsID = ads.AdsID,
                    Title = ads.Title,
